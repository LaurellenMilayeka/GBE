...
void CPU::Z80::Tick()
{
  ...
  printf("[DEBUG] : DIV Register before function : 0x%02X\n", Engine::RAM::GetByte(0xFF04));
  printf("[DEBUG] : Clock 8 lower bits : 0x%02X\n", clock & 0xFF);
  printf("[DEBUG] : Clock 8 lower bits after addition : 0x%02X\n", (clock & 0xFF) + this->nextInstructionDuration);
  ...
  if (((((clock & 0xFF) & 0xFF) + (this->nextInstructionDuration & 0xFF)) & 0x100) == 0x100)
  {
    ...
    printf("[DEBUG] : Carry flag set, incrementing DIV register\n");
  ...
  if (((Engine::RAM::GetByte(0xFF07) >> 2) & 1) == 1)
  {
    ...
    printf("[DEBUG] : TIMA Frequency  : %dHz\n", frequency);
  ...
  printf("[DEBUG] : DIV Register after function : 0x%02X\n", Engine::RAM::GetByte(0xFF04));
  printf("[DEBUG] : TIMA Register after function : 0x%02X\n", Engine::RAM::GetByte(0xFF05));
